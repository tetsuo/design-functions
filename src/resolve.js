...
function onresolve(cb) {
  return (er, deps) => {
    ...
    if (!deps.length) {
      return cb(new Error('Could not resolve modules for an unknown reason'))
...
export default (files, babelifyOptions, cb) => {
  if (!files.length) {
  ...
  collect(
    ...
    onresolve((er, res) => {
      ...
      fs.unlink(entryFile, er => {
        if (er) {
          return cb(new Error('Error while removing temporary module entry file: ' + entryFile))
        }
        cb(null, res)
      })
    })
  ...
  fs.writeFile(entryFile, files.map(file => `require('${file}');`).join('\n'), er => {
    if (er) {
      return cb(new Error('Error while writing temporary module entry file: ' + entryFile))
    }
